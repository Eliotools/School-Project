...
Droid::Droid(std::string s_nb) : Id(s_nb), Energy(50), Attack(25), Toughness(15)//, BattleData(new DroidMemory)
{
...
void Droid::setEnergy(const size_t n_Energy)
{
    this->Energy = n_Energy;
}

bool Droid::operator==(Droid const &fs)
{
    if (*fs.Status == *this->Status)
        return true;
    return false;
}
...
void Droid::setBattleData(DroidMemory *D_m)
{
    this->BattleData = D_m;
}